...
export default function PopupNotice() {
  ...
  const cmsPopup = cms?.popup
  const cmsNotice = cms?.notices?.[0]
  const activePopup = cmsPopup
    ? {
        ...popup,
        active: cmsPopup.active,
        badge: cmsPopup.badge,
        title: cmsPopup.title,
        description: cmsPopup.description,
        image: cmsPopup.image || '',
        cta: { label: cmsPopup.ctaLabel, url: cmsPopup.ctaUrl },
      }
    : cmsNotice
      ? {
          ...popup,
          active: true,
          badge: cmsNotice.badge,
          title: cmsNotice.title,
          description: cmsNotice.description,
          cta: { label: cmsNotice.ctaLabel || '자세히 보기', url: cmsNotice.ctaUrl || '/location' },
        }
      : popup
  ...
  if (!activePopup.active) return null
      ...
      {visible && (
        <>
          ...
          <motion.div
            ...
            onClick={activePopup.closable ? handleClose : undefined}
          />
            ...
            <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="px-6 py-5">
                ...
                {activePopup.cta && (
                  ...
                  >
                    {activePopup.cta.label}
                  </a>
                )}
              </div>

              {/* 하단 : 오늘 하루 보지 않기 */}
              {activePopup.doNotShowTodayOption && (
